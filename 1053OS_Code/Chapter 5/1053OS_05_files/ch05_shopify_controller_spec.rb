...
describe ShopifyController do
  ...
  context "confirm" do
    ...
    before do
      ...
      @account = FactoryGirl.create(:account)
      @accepted_response = OpenStruct.new(status: "accepted", id: 1231231)
      @rejected_response = OpenStruct.new(status: "pending", id: 1231231)


    end

    it "should activate the charge and mark the account as paid" do

      Account.should_receive(:find_by_charge_id).and_return(@account)
      ShopifyAPI::RecurringApplicationCharge.should_receive(:find).and_return(@accepted_response)
      @accepted_response.should_receive(:activate)
      @account.should_receive(:update_attribute).with(:paid, true)
      get :confirm, {:charge_id => "1"}, valid_session


    end

    it "should mark the account as free" do
      Account.should_receive(:find_by_charge_id).and_return(@account)
      ShopifyAPI::RecurringApplicationCharge.should_receive(:find).and_return(@rejected_response)
      @account.should_receive(:update_attribute).with(:paid, false)
      get :confirm, {:charge_id => "1"}, valid_session
